...
长对话经常在话题之间跳转，按固定轮数切块很容易把一段讨论拦腰截断。Neroli 会冻结已经稳定的事件卡，但在后续 turns 到来时重喂尾部的重叠原文，改写最新一张卡。这样卡片边界就能随着话题的展开或切换而调整，尽量让每张卡覆盖一段完整、连贯的话题，而不是机械地服从窗口长度。
...
- **解耦、可验证的架构。** 七个模块通过带版本的 SQLite contract 交换数据，后台处理与来源解耦；可见性过滤集中在存储层，MCP 只提供有界只读接口，并由 **147 项自动化测试**覆盖关键行为。
